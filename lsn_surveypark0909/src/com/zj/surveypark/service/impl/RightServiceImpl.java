...
public class RightServiceImpl extends BaseServiceImpl<Right> implements RightService {
	...
	public void saveOrUpdateRight(Right model) {
		...
		if (model.getId() == null) {
			...
			long rightCode = 1;
			// 查询最大权限位上的最大权限码
			// String hql="from Right r order by r.rightPos desc,r.rightCode
			// desc";
			...
			/*
			 * 空表 if(!ValidateUtil.isValid(list)){ rightPos=0; rightCode=1;
			 * }else{ Right topRight=list.get(0); int
			 * topRightPos=topRight.getRightPos(); long
			 * topRightCode=topRight.getRightCode(); //权限码是否到达最大值
			 * if(topRightCode>=(1L<<60)){ rightPos=topRightPos+1; rightCode=1;
			 * }else{ rightPos=topRightPos; rightCode=topRightCode<<1; } }
			 */
			if (topRightPos == null) {
				rightPos = 0;
				rightCode = 1;
	 ...
	 * if(!ValidateUtil.isValid(rights)){ return null; }else{ String temp="";
	 * for(Right r:rights){ temp=temp+r.getId()+","; } return
